#ifndef UNITEPATHSEARCH_H
#define UNITEPATHSEARCH_H


#include "merged_path_search.h"
...
bool PairPathInfoSorter(PairPathInfo PI1, PairPathInfo PI2)
{
    return (PI1.Exons<PI2.Exons || (PI1.Exons == PI2.Exons && PI1.sampleid<PI2.sampleid));
}
class MergeSample{
  public: 
	  string Strand, Chr;
...
//	  typedef icl::split_interval_map<int32_t, int32_t, icl::partial_absorber, less, icl::inplace_plus, icl::inter_section, ROI> split_interval_map;
	  ...
	  void get_pairpaths( vector<PairPathInfo > PairPathExon,vector<path_t>& Unused_pair_paths,vector<double> & Unused_pair_paths_cov,
                         vector<int> exon_l, vector<int> exon_r,
                         vector<int> junc_l, vector<int> junc_r, vector<double> Vec_weights);
				  ...
				  vector<pair<int,int> >& coordinates_of_one_sample)//graph corordinates
{
	...
	string s = file[i];
		...
		sort(ppaths.begin(),ppaths.end());
		ppaths.erase(unique(ppaths.begin(),ppaths.end()), ppaths.end());
		for(size_t i=0;i<ppaths.size();i++)
		{
  		    path_t p = ppaths[i];
		    vector<int> exons;
		    for(size_t j=0;j<p.size();j++) {
			exons.push_back(exon_l[p[j]]);
			exons.push_back(exon_r[p[j]]);
		    }
		    //for(size_t j=0;j<exons.size();j++) cout<<exons[j]<<" + ";
		    //cout<<endl;
		    PairPathInfo pi={exons,sample_id};
		    PairPaths.push_back(pi);
		}
...
void MergeSample::get_pairpaths( vector<PairPathInfo > PairPathExon,vector<path_t>& Unused_pair_paths,vector<double> & Unused_pair_paths_cov,
				 vector<int> exon_l, vector<int> exon_r,
			 	 vector<int> junc_l, vector<int> junc_r, vector<double> Vec_weights)
{
    //cout<<"PairPathExon size: "<<PairPathExon.size()<<endl;
    ...
    for(size_t i=0;i<PairPathExon.size();i++)
    {
	vector<int> exons = PairPathExon[i].Exons;
	path_t p;
	if(exons.size() < 6 ) continue;
	for(size_t i1=0;i1<exons.size();)
	{
	...
	for(size_t j=0;j<p.size()-1;j++)
	{
	    junc_t junc_ = make_pair(exon_r[p[j]] , exon_l[p[j+1]]);
	    vector<pair<int,int> >::iterator it = find(junc.begin(),junc.end(),junc_);
	    if(it == junc.end())
	    {
	        nojunc_flag = true;
				 ...
				 vector<PairPathInfo> pairpaths)
{
    ...
    cout<<"Pair: "<<pairpaths.size()<<endl;
    for(int a=0;a<pairpaths.size();a++)
    {
	vector<int> exons = pairpaths[a].Exons;
	for(int b=0;b<exons.size();b++) cout<<exons[b]<<" + ";
	cout<<" sample: "<<pairpaths[a].sampleid<<endl;
    }
    cout<<endl;
    */
    ...
    vector<double> Unused_pair_paths_cov;
    get_pairpaths(pairpaths,Unused_pair_paths,Unused_pair_paths_cov,
	     exon_l, exon_r,
	     junc_l, junc_r,edge_weight);
/*
    cout<<"Nodes: "<<exon.size()<<endl;
    ...
    for(int a=0;a<exon.size();a++)
     ...
     cout<<"PairPaths: "<<endl;
     for(size_t i=0;i<Unused_pair_paths.size();i++) {
	path_t p = Unused_pair_paths[i];
	for(size_t j=0;j<p.size();j++) cout<<p[j]<<"->";
	cout<<Unused_pair_paths_cov[i]<<endl;
    }
...
*/     
     ...
     ps.path_search();
//cout<<"searching paths finish!"<<endl;
...
void MergeSample::process_each_gene()
{
	...
	size_t n = 0;
	sort(PairPaths.begin(),PairPaths.end(),PairPathInfoSorter);
	for(size_t i=0;i<GeneRanges.size();i++)
	{
	    ...
	    for(;n<PairPaths.size();n++)
	    {
		if(PairPaths[n].Exons.front() >= l && PairPaths[n].Exons.back() <= r) 
		    pairpaths.push_back( PairPaths[n]);
		else break;
	    }
	    get_meta_graph(junction,junction_cov, raw_exon, raw_exon_cov, exon, exon_cov,pairpaths);
	}
	//out_graph.close();
	return;
}
