...
//      packing_map
  ...
  for(size_t I=0;I<nodes_of_components.size();I++)//Liger
  {
	...
	int i = nodes[k];
        for(size_t j=0;j<node_set[i].children.size();j++)
	{
	    if(SampleSize > 1) //concider only on algnments provided CRM
	    {
	        if(partial(i,node_set[i].children[j].first)) continue; //6.24unstranded
	    }
    ...
    while(1 && !pairpath_current.empty()) //pair as seed
    {
	...
	*/
	...
	/*
	out<<chr<<"TIGLON."<<rg_index<<" "<<seed_cov<<" "<<final_paths.size()+1<<": ";
	for(size_t i=0;i<final_path.size()-1;i++)
	{
	    int n1 = final_path[i], n2 = final_path[i+1];
  ...
  void SimplifyGraph::output(string strand, string chr)
  {
	...
	//path_info pi = final_paths_info[i]; //Liger
	...
	if( 
	      (unstranded && (length >300 || (final_paths.size() == 1 && cov > 1) ) )
	    ||(!unstranded && (length >500 || (final_paths.size() == 1 && cov > 10) ) )	
	    ||(SampleSize == 1 && (final_paths.size() == 1 && cov > 1)) //CRM
	   )
	{
      	  out_gtf<<chr<<"	"<<"TIGLON"<<"	"
