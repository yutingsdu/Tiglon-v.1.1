...
if(!mergeMode) {
	...
	if(outfname!="stdout") {
		...
		if (f_out==NULL) GError("Error hhcreating output file %s\n", outfname.chars());
	}
/* YU
	fprintf(f_out,"# ");
...
*/
	...
	if (ftmp_in!=NULL) {
		...
		GMALLOC(linebuf, linebuflen);
	  	if(Yuflag)
		{
		    int nl, istr, tlen,t_id;
		    float tcov;
		    while(fgetline(linebuf,linebuflen,ftmp_in)) {
			sscanf(linebuf,"%d %d %d %d %g", &istr, &nl, &tlen, &t_id, &tcov);
			//cout<<"YU: "<<linebuf<<endl;
			istringstream istr;
		   	istr.str(linebuf);
			string temp, lable , chr ,strand;
			int l,r;
			istr>>chr>>temp>>lable>>l>>r>>temp>>strand;
			istr.clear();
			if(lable != "transcript") continue;
			if(nl == 2) single_number++;
			all_number++;
			if(nl == 2 && strand == "+") continue;
			//cout<<chr<<" "<<strand<<" "<<lable<<" "<<l<<" "<<r<<endl;
			chr += strand;
			pair<int,int> p = make_pair(l,r);
			if(ChrTransBorder_Map.find(chr) != ChrTransBorder_Map.end())
			    ChrTransBorder_Map[chr].push_back(p);
			else
			{
			    vector<pair<int,int> > V(1,p);
			    ChrTransBorder_Map[chr] = V;
			}
			//ChrTransBorder_Map
		    }
		}
	  ...
	  fclose(ftmp_in);
	  double R_flag = double(1.0*single_number / all_number);
	  if(R_flag > 0.5) Yuflag = false;
	  ftmp_in=fopen(tmpfname.chars(),"rt");
	  if(ftmp_in!=NULL) {
		...
		while(fgetline(linebuf,linebuflen,ftmp_in)) {
			...
			bool Flag = false;
			//if(tcov <= COV_YU) Flag = true;;//gingko
			...
			if(istr) { // this is a transcript
				...
				if( Yuflag && nl==2) //YU SPK
			   	{

				    fgetline(linebuf,linebuflen,ftmp_in);
				    istringstream istr;
                        	    istr.str(linebuf);
                        	    string temp1, temp2, lable , chr ,strand, Ch, T;
                        	    int l,r;
                        	    istr>>chr>>temp1>>lable>>l>>r>>temp2>>strand>>T;
				    Ch = chr;
				    if(strand == "+") strand = "-";
				    else if(strand == "-") strand = "+";
				    chr += strand;
				    vector<pair<int,int> > transborder = ChrTransBorder_Map[chr];
				    bool flag = check_overlap(l,r,transborder);
				    if(!Flag && !flag ) {
					stringstream ss;
					ss<<Ch<<"	"<<temp1<<"	"
					  <<lable<<"	"<<l<<"	"<<r<<"	"<<temp2<<"	"
					...
					vector<string> Vs;
					string temp;
					while(istr>>temp) Vs.push_back(temp);
					for(size_t i=0;i<Vs.size()-1;i++)ss<<Vs[i]<<" ";
					ss<<Vs.back();
				    	string S = ss.str();
					fprintf(f_out,"%s",const_cast<char *>(S.c_str()));//linebuf);
					...
					if(Flag || flag) continue;
					    ...
					    ss<<Vs.back();
					    string S = ss.str();
					    fprintf(f_out,"%s\n",const_cast<char *>(S.c_str()));//linebuf);
					}
				 	istr.clear();
				    }
